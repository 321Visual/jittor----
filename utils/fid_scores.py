...
# from utils.jittor_fid.inception import InceptionV3
# import matplotlib.pyplot as plt
#
#
# # --------------------------------------------------------------#
# #  通过修改 https://github.com/mseitzer/pytorch-fid 的代码得到本代码
# # --------------------------------------------------------------#
...
#         mu, sigma = jittor.mean(pool, 0), jittor_cov(pool, rowvar=False)
...
#         with jittor.no_grad():
#             for i, data_i in enumerate(self.val_dataloader):
#                 image, label = preprocess_input(self.opt, data_i)
#                 if self.opt.no_EMA:
...
#
#     @jittor.single_process_scope()
#     def update(self, netG, netEMA, cur_iter):
#         print("--- Iter %s: computing FID ---" % (cur_iter))
#         cur_fid = self.compute_fid_with_valid_path(netG, netEMA)
...
#     m -= jittor.mean(m, dim=1, keepdims=True)  # todo keepdim 改为了keepdims 记得检测是否可行
#     mt = m.transpose()  # if complex: mt = m.t().conj()
#     temp = fact * m.matmul(mt)
#     return temp
import os
import numpy as np
import jittor as jt
...
class fid_jittor():
    def __init__(self, opt, dataloader_val):
        ...
        block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[self.dims]
        # 不使用 fid inception
        self.model_inc = InceptionV3(output_blocks=[block_idx])
        self.val_dataloader = dataloader_val
        self.m1, self.s1 = self.compute_statistics_of_val_path(dataloader_val)
        self.best_fid = 99999999
        self.path_to_save = os.path.join(self.opt.checkpoints_dir, self.opt.name, "FID")
